...
$sql = "SELECT * FROM message WHERE receiver_id = '$user_id' ORDER BY messageDateTime DESC";
   ...
   <div class="center-container">
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            //zjištění odesílatele zprávy
            $sender_id = $row['sender_id'];
            $sql_sender_email = "SELECT email FROM user WHERE id = $sender_id";
            $sender_email_result = $conn->query($sql_sender_email);
            $sender_mail = $sender_email_result->fetchColumn(); 

            //rošifrování zprávy
            $key = 'VpO7zGH4YLP7WHDGjEjqMmG0nDqp4JaO'; 
            $decryptedText = decryptText($row['text'], $key, $row['iv']);

            echo "<p>Sender: " . $sender_mail . "</p>";
            echo "<p>Text: ";
            if ($row['isLookedByUser'] == 0) {
                echo "<b>" . $decryptedText . "</b>"; // Pokud zpráva nebyla zobrazena, zobrazíme ji tučně
            } else {
                echo $decryptedText;
            }
            echo "</p>";
    ...
    $update_query = "UPDATE message SET isLookedByUser = 1 WHERE receiver_id = $user_id AND isLookedByUser = 0";
    // Provedení dotazu na to že se zprávy označí jako přečtené v databázi
